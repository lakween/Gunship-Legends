...
export default function GunshipLegend() {
  ...
  const resetGame = useCallback(() => {
    gs.current.score=0; scoreSubmitted.current=false; startedRef.current=false; overRef.current=false;
    gs.current=mkGs(); gs.current.score=0; setScore(0); setGameOver(false); setGameStarted(false);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const player = buildPlayerHeli(PIXI); player.x=gs.current.heli.x; player.y=gs.current.heli.y; gameLayer.addChild(player);
      ...
      app.ticker.add(()=>{
        ...
        player.x=s.heli.x; player.y=s.heli.y;
        ...
        if(s.spawnTimer>=s.spawnRate){
          ...
          const ec=BUILDERS[type](PIXI); ec.x=W+16; ec.y=ey; gameLayer.addChild(ec); enemyMap.set(eid,ec);
        }
        ...
        s.enemies.forEach(e=>{
          ...
          if(ec){
            // Idle bob + position
            ec.x=e.x; ec.y=e.y+Math.sin(e.wobble*0.04)*3.5;
        ...
        if(s.invincible>0){s.invincible--;}
          ...
          if(hit){s.hp--;s.invincible=CFG.player.invincibleFrames;spawnExplosion(s,s.heli.x+42,s.heli.y+22,s.hp===0);s.shake.intensity=s.hp===0?20:11;if(s.hp<=0)setTimeout(triggerGameOver,360);}
        }
