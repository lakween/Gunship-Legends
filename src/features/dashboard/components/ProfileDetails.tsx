...
function getLeagueInfo(bestScore: number) {
  ...
  const progress = isMaster
    ? 100
    : Math.round(((bestScore - current.min) / (current.max! + 1 - current.min)) * 100);
      ...
      <div className="flex flex-col gap-5 p-6 overflow-y-auto max-h-[calc(100vh-220px)]">
        ...
        <div className={`rounded-xl border ${league.border} ${league.bg} p-4`}>
          <div className="flex items-center justify-between mb-2">
            ...
            {isMaster && (
              <span className="text-xs text-purple-400 font-bold">Max Rank 👑</span>
            )}
          </div>
          ...
          <div className="h-2 w-full bg-black/30 rounded-full overflow-hidden">
            <div
              className={`h-full bg-gradient-to-r ${league.color} rounded-full transition-all duration-700 ease-out`}
