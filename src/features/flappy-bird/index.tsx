...
export default function FlappyBird() {
    ...
    const scoreRef = useRef(0);           // always holds latest score for game loop
    const scoreSubmitted = useRef(false); // prevents double-submit per round
    ...
    // ── Submit score to API then set game over ──────────────────────────
    ...
    useEffect(() => {
        ...
        const runLoop = () => {
            ...
            if (gameStarted && !gameOver) {
                ...
                s.pipes.forEach(p => {
                    ...
                    if (50 + 20 > p.x && 50 < p.x + 50 &&
                        ...
                        triggerGameOver(); // ← API call + setGameOver
                    }
                    if (p.x < 50 && !p.passed) {
                ...
                if (s.birdY > GROUND_Y - 10 || s.birdY < 0) triggerGameOver();
            }
            ...
            if (gameStarted) {
                ...
                ctx.fillText(String(scoreRef.current), W / 2 + 2, 52);
                ctx.fillStyle = "white";
    ...
    }, [gameStarted, gameOver, triggerGameOver]);
    ...
    useEffect(() => {
        const handleAction = (e?: KeyboardEvent) => {
            ...
            if (gameOver) return resetGame();       // ← resetGame instead of reload
